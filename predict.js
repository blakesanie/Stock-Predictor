...
request(url, { json: true }, (err, res, body) => {
    ...
    var input = index.slice(index.length - daysBefore);
    ...
    plot(index.concat(futureValues), index.length);
});
...
function plot(predictedValues, daysBeforePresent) {
    var xs = [];
    for (var i = 0; i < predictedValues.length; i++) {
        xs.push(-1 * daysBeforePresent + i + 1);
    }
    var trace1 = {
        x: xs,
        y: predictedValues,
    ...
    plotly.getImage(figure, imgOpts, function (error, imageStream) {
        ...
        var fileStream = fs.createWriteStream(symbol + "_next_" + daysInFuture + "_" + timeSeries + '.png');
